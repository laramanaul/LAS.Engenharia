  ...
  methods: {
    ...
    async carregarProjetos() {
      if (!this.organizacaoId || !this.user?.uid) {
        console.warn('🚫 Organização ou usuário não definidos');
        return;
      }

      console.log('📌 carregarProjetos → organizacaoId:', this.organizacaoId);
      console.log('👤 carregarProjetos → user.uid:', this.user.uid);

      try {
        const querySnapshot = await getDocs(collection(db, 'projetos'));
        const todosProjetos = querySnapshot.docs.map(doc => {
          const data = { id: doc.id, ...doc.data() };
          console.log('🔥 Projeto bruto do Firestore:', data);
          return data;
        });

        const filtrados = todosProjetos.filter(p => {
          const orgId = (p.organizacaoId || '').trim();
          const criadoPor = (p.criadoPor || '').trim();
          const allowedUsers = Array.isArray(p.allowedUsers) ? p.allowedUsers : [];

          const pertenceOrganizacao = orgId === this.organizacaoId.trim();
          const autorizado = criadoPor === this.user.uid || allowedUsers.includes(this.user.uid);

          if (!pertenceOrganizacao || !autorizado) {
            console.warn('❌ Projeto ignorado:', {
              id: p.id,
              orgId,
              criadoPor,
              allowedUsers,
          ...
          return pertenceOrganizacao && autorizado;
        });

        this.projetos = filtrados;

        console.log('✅ Projetos carregados (finais):', this.projetos);
        if (!this.projetos.length) {
          console.warn('⚠️ Nenhum projeto carregado após filtragem');
        }
      } catch (err) {
